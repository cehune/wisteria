...
static_assert(sizeof(Material)             == 96, "Material layout changed");
...
static_assert(sizeof(Light)                == 48, "Light layout changed");
...
void AccelStructures::_buildTLAS(Scene& scene) {
    ...
    for (NS::UInteger i = 0; i < n; ++i) {
        ...
        instData[i].indexCount  = scene.meshes()[inst.meshIndex].numTriangles * 3;
